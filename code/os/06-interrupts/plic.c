...
void plic_init(void)
{
	 ...
	 * effective priority.
	 */
	*(uint32_t *)PLIC_PRIORITY(UART0_IRQ) = 1; // 设置UART的核心的优先级别
	 ...
	 */
	*(uint32_t *)PLIC_MENABLE(hart) = (1 << UART0_IRQ); // 设置UART那一位的权限是可以的
	 ...
	 */
	*(uint32_t *)PLIC_MTHRESHOLD(hart) = 0; // 设置可以接受的终端的最低阈值为0

	/* enable machine-mode external interrupts. */
	w_mie(r_mie() | MIE_MEIE); // 这里先读然后再写入,保证
 ...
 * RETURN VALUE: none
 */
void plic_complete(int irq) // 告诉PLIC做完了
{
	int hart = r_tp();
